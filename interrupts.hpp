...
std::pair<std::string, int> intr_boilerplate(int current_time, int intr_num , int context_save_time, int negligible_time,std::vector<std::string> vectors) {
    
    //Intiialize empty execution string
    std::string execution = "";

    // Switch to/from kernel mode
    execution += std::to_string(current_time) + ", " + std::to_string(negligible_time) + ", switch to kernel mode\n";
    ...
    //Calculate ISR Address
    execution += std::to_string(current_time) + ", " + std::to_string(negligible_time) + ", find vector " + std::to_string(intr_num) 
